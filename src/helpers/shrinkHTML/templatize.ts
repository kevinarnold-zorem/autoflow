...
function removeElements(root: Element) {
  // Eliminar elementos <div> del DOM
  ...
  // Eliminar las etiquetas <html>, <head> y <body> del DOM
  ...
  if (bodyElement) {
    while (bodyElement.firstChild) {
      root.insertBefore(bodyElement.firstChild, bodyElement);
    }
  ...
  if (headElement) {
    while (headElement.firstChild) {
      root.insertBefore(headElement.firstChild, headElement);
    }
    root.removeChild(headElement);
  }

  if (htmlElement) {
    while (htmlElement.firstChild) {
      root.appendChild(htmlElement.firstChild);
    }
    root.removeChild(htmlElement);
  }

  // Eliminar los elementos <td> de las tablas, dejando solo los elementos clicables
  const tableCells = root.querySelectorAll('td');
  tableCells.forEach((tableCell) => {
    const buttonElement = tableCell.querySelector('button, a');
    if (!buttonElement) {
      const parentElement = tableCell.parentElement;
      while (tableCell.firstChild) {
        parentElement?.insertBefore(tableCell.firstChild, tableCell);
      }
      parentElement?.removeChild(tableCell);
    }
  });

  // Eliminar los elementos <lightning-accordion> del DOM y su contenido
  const accordionElements = root.querySelectorAll('lightning-accordion');
  accordionElements.forEach((accordionElement) => {
    accordionElement.parentElement?.removeChild(accordionElement);
  });

  // Eliminar los elementos <lightning-datatable> del DOM y su contenido
  const datatableElements = root.querySelectorAll('lightning-datatable');
